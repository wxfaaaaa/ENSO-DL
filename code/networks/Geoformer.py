...
class Geoformer(nn.Module):
    def __init__(self, mypara):
        ...
        if self.mypara.needtauxy:
            self.cube_dim = ((mypara.input_channal + 2) * mypara.patch_size[0] * mypara.patch_size[1])
        else:
        ...
        if self.mypara.specific_time:
            self.out_len = 1
        else:
            self.out_len = mypara.output_length
        ...
        enc_layer = miniEncoder(d_size, mypara.nheads, mypara.dim_feedforward, mypara.dropout)
        dec_layer = miniDecoder(d_size, mypara.nheads, mypara.dim_feedforward, mypara.dropout)
        self.encoder = multi_enc_layer(enc_layer=enc_layer, num_layers=mypara.num_encoder_layers)
        self.decoder = multi_dec_layer(dec_layer=dec_layer, num_layers=mypara.num_decoder_layers)
        self.linear_output = nn.Linear(d_size, self.cube_dim)

    def forward(self, predictor, predictand, in_mask=None, enout_mask=None, train=True, sv_ratio=0,):
        """
        Args:
            predictor: (batch, lb, C, H, W) == input_var
        ...
        if train:
            with torch.no_grad():
                connect_inout = torch.cat([predictor[:, -1:], predictand[:, :-1]], dim=1) # 这里是11-30，
                out_mask = self.make_mask_matrix(connect_inout.size(1))
                if self.need_attn:
                    outvar_pred, de_time_attn_weights, de_space_attn_weights = self.decode(connect_inout, en_out, out_mask, enout_mask,)
                else:
                    outvar_pred = self.decode(connect_inout, en_out, out_mask, enout_mask,)
            if sv_ratio > 1e-7:
                supervise_mask = torch.bernoulli(sv_ratio * torch.ones(predictand.size(0), predictand.size(1) - 1, 1, 1, 1)).to(predictand.device)
            else:
                supervise_mask = 0
            predictand = (supervise_mask * predictand[:, :-1] + (1 - supervise_mask) * outvar_pred[:, :-1])
            ...
            predictand = predictor[:, -1:] # 提取最后一个时间步长
    ...
    def decode(self, predictand, en_out, out_mask, enout_mask):
        ...
        predictand = predictand.reshape(predictand.size(0), T, self.cube_dim, -1).permute(0, 3, 1, 2)
        ...
        output = output.reshape(predictand.size(0), T, self.cube_dim, H // self.mypara.patch_size[0], W // self.mypara.patch_size[1],)
...
class multi_enc_layer(nn.Module):
    def __init__(self, enc_layer, num_layers, need_attn=False):
        super().__init__()
...
class multi_dec_layer(nn.Module):
    def __init__(self, dec_layer, num_layers, need_attn=False):
        super().__init__()
        self.need_attn = need_attn
